...
class BLS_Scanner {
    ...
    private function node_should_skip( DOMElement $node ): bool {
        ...
        // WooCommerce block output driven by the Interactivity API.
        //
        // The Product Collection block renders each tile's link as:
        //   <a data-wp-on--click="woocommerce/product-collection::actions.viewProduct"
        //      href=".../product/core/">Core</a>
        // A product grid placed on a page therefore contributed one row per
        // tile, each reported as missing an SEO title — but the markup is the
        // block's, not the author's. There is no anchor in the page content to
        // add a title to, so the row can never be actioned.
        //
        // Matched on the directive's namespace, which is generated by
        // WooCommerce and survives any amount of restyling. Scoped to
        // woocommerce/ deliberately: other blocks use the same directive
        // syntax, and this is not the place to make assumptions about them.
        foreach ( $node->attributes as $attribute ) {
            if ( strpos( strtolower( $attribute->nodeName ), 'data-wp-on-' ) !== 0 ) {
                continue;
            }
            if ( str_contains( strtolower( $attribute->nodeValue ), 'woocommerce/' ) ) {
                return true;
            }
        }

 ...
 *
 * Controls are a compact pill bar: one dropdown per filter, each opening a
 * small popover. Override by copying to your theme as andyoei/directory.php.
 *
 * @var array  $config
 * @var array  $request
 * @var array  $result
...
<div class="ao-directory" data-ao-directory="<?php echo esc_attr( $key ); ?>" data-filtered="<?php echo $filtered ? '1' : '0'; ?>">
	...
	<div class="ao-bar">
		...
		<?php foreach ( $config['facets'] as $name => $facet ) : ?>
			...
			?>
			<div class="ao-pill ao-pill--facet" data-facet="<?php echo esc_attr( $name ); ?>">
				<button type="button" class="ao-pill-button" aria-expanded="false" aria-haspopup="true">
					...
					<span class="ao-pill-value"><?php echo esc_html( AO_Query::facet_summary( $facet, $terms, $active ) ); ?></span>
				...
				<div class="ao-menu" hidden>
					...
					<button type="button" class="ao-menu-clear">Clear</button>
				</div>
			</div>
		<?php endforeach; ?>

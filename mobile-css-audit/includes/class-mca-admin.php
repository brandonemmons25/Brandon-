        ...
        (function () {
            ...
            function renderOutput() {
                ...
                var matched = pageBlocks.filter(function (block) {
                    ...
                    if (filter === 'overflow'  && block.indexOf('OVERFLOW CULPRITS')    === -1) return false;
                    if (filter === 'collapsed' && block.indexOf('COLLAPSED ELEMENTS')   === -1) return false;
                    if (filter === 'issues'    && block.indexOf('OVERFLOW CULPRITS')    === -1
                                               && block.indexOf('COLLAPSED ELEMENTS')  === -1
                                               && block.indexOf('LAYOUT DIAGNOSIS')    === -1) return false;
                    if (filter === 'clean'     && (block.indexOf('OVERFLOW CULPRITS')   !== -1
                                               || block.indexOf('COLLAPSED ELEMENTS')  !== -1
                                               || block.indexOf('LAYOUT DIAGNOSIS')    !== -1)) return false;
                    return true;
                });
